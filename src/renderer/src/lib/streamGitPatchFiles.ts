// Incremental git-patch splitter ported from pierre's diffshub. Reads a byte
// stream of `git diff` / patch output and invokes `onFileText` once per file as
// soon as that file is fully buffered (i.e. the *next* `diff --git` header has
// arrived), so the viewer can parse and render files while the rest of the diff
// is still downloading. GitHub's combined `.diff` has no `From <hash>` mailbox
// headers, so the commit-metadata handling below is dormant for our source but
// kept intact so the parser also tolerates multi-commit `.patch` input.
...
function createGitPatchFileStreamParser(): GitPatchFileStreamParser {
  ...
  function takeAvailableFile(): string | undefined {
    if (currentFileBoundaryIndex == null) {
      currentFileBoundaryIndex = findNextGitFileBoundary(buffer, nextBoundarySearchIndex)
      if (currentFileBoundaryIndex == null) {
    ...
    for (;;) {
      ...
      const nextBoundaryIndex = findNextGitFileBoundary(buffer, nextBoundarySearchIndex)
      if (nextBoundaryIndex == null) {
        nextBoundarySearchIndex = getNextBoundarySearchIndex(buffer, fileBoundaryIndex + 1)
        return undefined
      }

      const splitIndex = getStreamedFileSplitIndex(buffer, fileBoundaryIndex, nextBoundaryIndex)
      ...
      nextBoundarySearchIndex = currentFileBoundaryIndex == null ? 0 : currentFileBoundaryIndex + 1
...
function getStreamedFileSplitIndex(text: string, firstBoundaryIndex: number, nextBoundaryIndex: number): number {
  return findLastCommitMetadataBoundary(text, firstBoundaryIndex + 1, nextBoundaryIndex) ?? nextBoundaryIndex
}

function findLastCommitMetadataBoundary(text: string, startIndex: number, endIndex: number): number | undefined {
  ...
  for (;;) {
    ...
    const line = text.slice(
      boundaryIndex,
      lineEndIndex === -1 || lineEndIndex > maximumBoundaryIndex ? maximumBoundaryIndex : lineEndIndex
    )
