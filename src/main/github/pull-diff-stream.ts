...
// Streams a pull request's raw unified diff to the renderer one network chunk
// at a time so the viewer can parse and render files as they arrive instead of
// waiting for the whole patch. The renderer generates the streamId, subscribes
// to the chunk/end/error channels, then invokes the start handler — so there is
// no subscribe-after-first-chunk race.
...
export function registerPullDiffStreamHandlers(): void {
  ipcMain.handle(
    ...
    async (event, streamId: string, owner: string, repo: string, number: number): Promise<void> => {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 401) {
            ...
            fail(`The diff for PR #${number} is too large to stream.`, true)
          } else {
            fail(`Failed to load the diff for PR #${number} (${response.status}).`)
          }
          return
        }

        if (!response.body) {
          fail(`GitHub returned an empty diff for PR #${number}.`)
          return
        }
        ...
        fail(error instanceof Error ? error.message : `Failed to load the diff for PR #${number}.`)
      } finally {
